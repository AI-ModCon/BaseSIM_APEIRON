        ...
        [
            ...
            transforms.Lambda(
                lambda x: transforms.functional.crop(
                    x, input_start_y, input_start_x, crop_size, crop_size
                )
            ),
            transforms.ToTensor(),
...
def get_gs_train(root: str = "examples/gray_scott/data/train") -> datasets.ImageFolder:
    return datasets.ImageFolder(root, transform=_base_tf())


def get_gs_val(root: str = "examples/gray_scott/data/val") -> datasets.ImageFolder:
    return datasets.ImageFolder(root, transform=_base_tf())
