...
def register_solps2dwion_dataset(norm_envelopes: dict[str, Any] | None = None) -> None:
    """Register SOLPS2DwION (b2time.nc) with MATEY's dataset factory."""
    from matey.data_utils import datasets as matey_datasets

    from examples.matey.solps.solps2dwion_dataset import (
        SOLPS2DwIONDataset,
        register_envelopes,
    )

    if norm_envelopes:
        register_envelopes(norm_envelopes)
...
def _slice_sample_axis(batch: Any, n: int, sel: Any) -> Any:
    """Apply ``sel`` to every field that carries a sample axis of length ``n``.

    Which fields those are is decided per value rather than by name, because
    ``leadtime`` arrives either per sample or as the adapter's global ``[1, 1]``
    default. Anything else -- ``tkhead_name``, ``blockdict`` -- describes the
    batch as a whole and is passed through.
    """
    out = {}
    for f in fields(batch):
        value = getattr(batch, f.name)
        keep = (
            isinstance(value, torch.Tensor) and value.ndim > 0 and value.shape[0] == n
        )
        out[f.name] = value[sel] if keep else value
    return replace(batch, **out)


def _cat_sample_axis(a: Any, b: Any) -> Any:
    """Concatenate ``a`` and ``b`` field-wise along their sample axis."""
    na, nb = len(a), len(b)
    out = {}
    for f in fields(a):
        ...
        out[f.name] = torch.cat([va, vb], dim=0) if both else va
    return replace(a, **out)
...
class MateyInputBatch:
    ...
    def __len__(self) -> int:
        if self.is_graph or self.input is None:
            raise TypeError(
                "MateyInputBatch has no sample axis for a graph batch; replay "
                "and slicing are only defined for gridded batches."
            )
        return int(self.input.shape[0])

    def __getitem__(self, sel: Any) -> "MateyInputBatch":
        return cast("MateyInputBatch", _slice_sample_axis(self, len(self), sel))

    def _geometry_key(self) -> tuple:
        """What must match for two batches to be a single forward pass.

        ``tkhead_name`` selects the tokenizer head and with it the patch size,
        and ``Ind_dim`` is the block geometry the model reshapes tokens back
        into, so a mismatch in either would silently re-patch one half at the
        wrong resolution. Compared as a plain tuple of ints: ``Ind_dim`` may
        hold tensors, and ``==`` on those returns a tensor rather than a bool.
        """
        ind = (self.blockdict or {}).get("Ind_dim")
        return (
            self.tkhead_name,
            tuple(int(v) for v in ind) if ind is not None else None,
            None if self.input is None else tuple(self.input.shape[1:]),
        )

    def can_cat_with(self, other: "MateyInputBatch") -> bool:
        if self.is_graph or other.is_graph:
            return False
        return self._geometry_key() == other._geometry_key()

    @classmethod
    def cat(cls, a: "MateyInputBatch", b: "MateyInputBatch") -> "MateyInputBatch":
        return cast("MateyInputBatch", _cat_sample_axis(a, b))
...
class MateyTargetBatch:
    ...
    def __getitem__(self, sel: Any) -> "MateyTargetBatch":
        return cast("MateyTargetBatch", _slice_sample_axis(self, len(self), sel))

    @classmethod
    def cat(cls, a: "MateyTargetBatch", b: "MateyTargetBatch") -> "MateyTargetBatch":
        return cast("MateyTargetBatch", _cat_sample_axis(a, b))
