...
from plot_solps_drift_showcase import (  # noqa: E402
    C_BLUE, C_GREEN, C_ORANGE, C_PINK, C_SKY, C_VERM,
    GRID, INK, INK2, MUTED, TEXTWIDTH, corner_tag, save, set_style,
)
...
MAP_ORDER = ["ITER PFPO", "KSTAR RMP", "DIII-D PT",
             "DIII-D NT", "ASDEX-U", "ASDEX-U favB"]
MAP_FIELD = "e_den"

FIELD_TEX = {
    "e_den": r"$n_e$", "e_T_perp": r"$T_{e\perp}$", "e_T_para": r"$T_{e\parallel}$",
    "e_u_para": r"$u_{e\parallel}$", "i_T_perp": r"$T_{i\perp}$",
    "i_T_para": r"$T_{i\parallel}$", "i_u_para": r"$u_{i\parallel}$",
    "dpot": r"$\delta\phi$",
}
...
def coverage_self_inclusive(res: dict) -> dict:
    ...
    *score*: a pre-trained device is by definition covered, and excluding
    ...
    return {lab: float(np.min([w[lab][r] for r in refs if r in w[lab]],
                              axis=0).mean())
...
def draw_device(ax, npz, lab: str, fidx: int, vmin: float, vmax: float,
                show_ylabel: bool):
    ...
    tpc = ax.tripcolor(triang, val, shading="gouraud",
                       ...
                       rasterized=True)
    try:
        ax.tricontour(triang, psin, levels=[1.0], colors="#00d4ff",
                      linewidths=0.8, zorder=5)
    except Exception:  # noqa: BLE001 -- separatrix may fall outside the domain
        pass
    ax.add_collection(LineCollection(boundary_segments(rz, tri), colors=INK,
...
def fig5(res: dict, npz, outdir: Path) -> dict:
    ...
    labs = [l for l in MAP_ORDER if l in cases and cases[l]["has_snapshot"]]
    ...
    gs = fig.add_gridspec(2, 3, hspace=0.46, wspace=0.40,
                          left=0.08, right=0.86, top=0.815, bottom=0.10)
    tpc = None
    for n, lab in enumerate(labs):
        ...
        ax.text(0.03, 0.97, f"{info['mesh_nodes']:,} nodes",
    ...
    fig.legend(handles=[
        Line2D([], [], color=C_BLUE, lw=0, marker="s", ms=4,
               label="in XGC pre-training"),
        Line2D([], [], color=C_VERM, lw=0, marker="s", ms=4,
        ...
        bbox_to_anchor=(0.47, -0.005), columnspacing=1.6, handletextpad=0.5)
    ...
    return {"field": MAP_FIELD, "vmin": vmin, "vmax": vmax,
            "panels": labs,
            "mesh_nodes": {l: cases[l]["mesh_nodes"] for l in labs},
            "mesh_tri": {l: cases[l]["mesh_tri"] for l in labs}}
...
def fig6(res: dict, npz, outdir: Path) -> dict:
    ...
    labs = res["labels"]
    fields = res["fields"]
    dev = {l: res["cases"][l]["device"] for l in labs}
    same = res["same_machine_pairs"]
    cross = res["cross_machine_pairs"]

    fig = plt.figure(figsize=(TEXTWIDTH, 5.9))
    gs = fig.add_gridspec(2, 3, hspace=0.78, wspace=0.70,
                          top=0.87, bottom=0.10, left=0.085, right=0.975)
    ...
    groups = [
        ...
        ("temporal\nsame run",
         [x for v in lv["L0_temporal_windowed"].values() for x in v], C_SKY),
    ...
    for i, (name, vals, col) in enumerate(groups):
        ...
        v = v[np.isfinite(v)]
        means.append(v.mean())
        ax.scatter(i + rng.uniform(-0.16, 0.16, v.size), v, s=7, color=col,
                   alpha=0.55, lw=0, zorder=3)
        ax.hlines(v.mean(), i - 0.34, i + 0.34, color=col, lw=2.0, zorder=4)
    ax.set_ylim(min(means) * 0.35, max(means) * 3.4)
    for i, (name, vals, col) in enumerate(groups):
        ax.annotate(f"{means[i]:.3f}", xy=(i, means[i]),
                    xytext=(0, 7), textcoords="offset points",
                    ha="center", fontsize=5.8, color=col, fontweight="bold")
    ax.set_xticks(range(len(groups)))
    ax.set_xticklabels([g[0] for g in groups], fontsize=5.2, linespacing=0.95,
    ...
    start = 0
    for d in dict.fromkeys(dev[l] for l in labs):
        n = sum(1 for l in labs if dev[l] == d)
        if n > 1:
            ax.add_patch(mpatches.Rectangle((start - 0.5, start - 0.5), n, n,
                                            fill=False, ec=C_GREEN, lw=1.4,
                                            zorder=6))
        start += n
    ax.grid(False)
    ...
    ax.text(0.5, -0.62, "green outline: same machine", transform=ax.transAxes,
            ha="center", fontsize=5.2, color=C_GREEN)
    corner_tag(ax, "(b)", dx=-0.36)

    # --- (c) coverage vs. the pre-trained devices ------------------------
    ax = fig.add_subplot(gs[0, 2])
    cov = coverage_self_inclusive(res)
    order = [l for l in labs if l in cov and np.isfinite(cov[l])]
    vals = [cov[l] for l in order]
    cols = [C_BLUE if res["cases"][l]["in_pretraining"] else C_VERM for l in order]
    y = np.arange(len(order))
    ax.barh(y, vals, color=cols, edgecolor="white", lw=0.6)
    for i, v in enumerate(vals):
        ax.text(v + max(vals) * 0.03, i, f"{v:.2f}", va="center", fontsize=5.8,
                color=INK2)
    ax.set_yticks(y)
    ...
    for k in same_keys:
        ax.plot(centres, rad[k], color=C_GREEN, lw=1.2, alpha=0.85,
                label="same machine" if k == same_keys[0] else None)
    for k in cross_keys:
        ax.plot(centres, rad[k], color=C_VERM, lw=0.8, alpha=0.45,
                label="different machine" if k == cross_keys[0] else None)
    ax.axvline(1.0, color=INK2, ls="--", lw=0.8)
    ax.text(0.985, 0.45, "separatrix", fontsize=5.2, color=INK2,
    ...
    ax.legend(fontsize=5.4, loc="center left", borderpad=0.3,
    ...
    ax.barh(y - 0.2, sm, height=0.38, color=C_GREEN, label="same machine",
            edgecolor="white", lw=0.4)
    ax.barh(y + 0.2, cm, height=0.38, color=C_VERM, label="different machine",
            edgecolor="white", lw=0.4)
    ax.set_yticks(y)
    ...
    ax.scatter(pu[~iscross], pv[~iscross], s=16, color=C_GREEN,
               label="same machine", zorder=3, lw=0)
    ax.scatter(pu[iscross], pv[iscross], s=16, color=C_VERM,
               label="different machine", zorder=3, lw=0)
    ...
    fig.suptitle("Same-machine vs. different-machine drift on XGC gyrokinetic data",
                 fontsize=8.5, y=0.975)
    save(fig, outdir, "fig6_xgc_same_vs_cross")

    return {
        "levels": {k: v for k, v in lv.items()
                   if isinstance(v, (int, float)) or k.startswith("cross")
                   or k.startswith("same")},
        "same_machine_pairs": [{"pair": f"{r['a']} | {r['b']}",
                                "ks_windowed": r["ks_windowed"],
                                "ks_pooled": r["ks_pooled"]} for r in same],
        "cross_machine_pairs": [{"pair": f"{r['a']} | {r['b']}",
                                 "ks_windowed": r["ks_windowed"],
                                 "ks_pooled": r["ks_pooled"]} for r in cross],
...
def fig7(res: dict, det: dict, outdir: Path) -> dict:
    ...
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 0.95], hspace=0.72,
                          wspace=0.30, top=0.87, bottom=0.11,
    ...
    ax.scatter(x[:boundary], stream[:boundary], s=11, color=C_BLUE, zorder=4,
               lw=0, label="in XGC pre-training")
    ax.scatter(x[boundary:], stream[boundary:], s=11, color=C_VERM, zorder=4,
               lw=0, label="held out from XGC branch")
    prev = 0
    for b in ds["bounds"]:
        if b["end"] < len(stream):
            ax.axvline(b["end"] - 0.5, color=GRID, lw=0.7, zorder=1)
        ax.text((prev + b["end"] - 1) / 2, ax.get_ylim()[1], b["label"],
                ha="center", va="bottom", fontsize=4.8,
                color=C_BLUE if b["in_pretraining"] else C_VERM, rotation=0)
        prev = b["end"]
    ...
    for i, (n, d) in enumerate(zip(names, delays)):
        if d is None:
            ...
            ax.text(cap * 0.5, i, "never fires", ha="center", va="center",
                    fontsize=5.4, color=INK2)
        else:
            ax.barh(i, d, color=C_GREEN, edgecolor="white", lw=0.5)
            ax.text(d + cap * 0.03, i, f"{d}", va="center", fontsize=5.8,
                    color=INK2)
    ax.set_yticks(range(len(names)))
    ax.set_yticklabels([n.replace(" as-shipped", "\nas-shipped").replace(
        " resized", "\nresized") for n in names], fontsize=5.0, linespacing=0.9)
    ax.invert_yaxis()
    ...
    if ctrl is not None:
        ...
        ax.set_title(f"control: {ctrl['reference']} $\\rightarrow$ "
                     f"{ctrl['second_case']}\n(same machine, scenario change only)",
                     fontsize=6.4, pad=3)
        nfire = sum(len(v["fired"]) for v in ctrl["detectors"].values())
        ax.text(0.03, 0.93, f"{sum(1 for v in ctrl['detectors'].values() if v['fired'])}"
                f"/{len(ctrl['detectors'])} detectors fire "
                f"({nfire} detections)",
                transform=ax.transAxes, ha="left", va="top", fontsize=5.6,
                color=C_GREEN, fontweight="bold")
        ax.text(cb - 0.4, ylim_a[1] * 0.52, " scenario change", fontsize=5.0,
                color=C_GREEN, va="center")
    corner_tag(ax, "(c)", dx=-0.22)

    fig.suptitle("APEIRON detector response on the corrected XGC stream",
                 fontsize=8.5, y=0.975)
    ...
    return {
        ...
        "device_false_alarms": {n: ds["detectors"][n]["false_alarms"]
                                for n in names},
        "control_fires": ({n: len(ctrl["detectors"][n]["fired"])
                           for n in ctrl["detectors"]} if ctrl else None),
    }
...
def main() -> int:
    ...
    if detpath.exists():
        ...
        print(f"  [skip] fig7: {detpath} not found "
              f"(run xgc_detector_sweep.py first)")
