...
# %j logs land in the submitting directory; .gitignore keeps them out of the tree.
#SBATCH --output=slurm-matey-oracle-%j.out
#SBATCH --error=slurm-matey-oracle-%j.err
...
CKPT="${CKPT:?set CKPT}"
ORACLE_DIR="${ORACLE_DIR:?set ORACLE_DIR to where the oracle checkpoint should be written}"
