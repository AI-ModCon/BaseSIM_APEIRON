...
class MATEYStreamHarness(MATEYHarness):
    ...
    def update_data_stream(self) -> None:
        ...
        # Record the arrival index into the metrics CSV as well as the console.
        # The console banner carries the logger's step counter, which is not the
        # one the metrics backend advances, so every banner reports step=0 and
        # the run log cannot be used to place arrivals on the metric axis.
        # increment=False: this annotates the current step rather than consuming
        # one, so it cannot shift the metrics that follow.
        logger.log({"stream/arrival": idx}, commit=False, prefix=False, increment=False)
