...
from examples.cifar10.utils import (
    get_cifar10_train,
    get_cifar10_val,
    FixedAffine,
    TransformedView,
...
class Vit16b(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Loads pretrained backbone and adapts the classification head to `num_labels`
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        Returns: [B, num_labels] log-probabilities (for NLLLoss).
        """
    ...
    def encode(self, x: Tensor) -> Tensor:
        """
        Optional: return CLS embeddings from the ViT encoder for analysis.
        """
        vit = getattr(self.model, "vit", None)
        if vit is None:
            # Some checkpoints wrap the backbone differently; fall back to logits proj if needed
            out = self.model(
                pixel_values=x, return_dict=True, output_hidden_states=True
            )
            return out.hidden_states[-1][:, 0, :]  # CLS from last hidden state
        out = vit(pixel_values=x, return_dict=True)
        return out.last_hidden_state[:, 0, :]  # CLS token


class CIFAR10_VIT16B(BaseModelHarness):
    """
    Pattern:
    ...
    def __init__(self, cfg: Config, model: nn.Module = Vit16b()):
        super().__init__(cfg=cfg, model=model)

        # FULL datasets (no index split)
        self.ds_train = get_cifar10_train("./data", normalize=True)
        self.ds_val = get_cifar10_val("./data", normalize=True)
