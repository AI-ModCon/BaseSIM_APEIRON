...
class VisionModelImageNet(nn.Module):
    """
    ViT-B/16 classifier for ImageNet using a Hugging Face pretrained checkpoint.

    - Expects inputs already resized to 224x224 and normalized.
    - Outputs log-probabilities (for use with NLLLoss).
    """
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"Warning: Pretrained model not found at {pretrained_path}. Using huggingface weights"
            )
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        Forward pass for the VisionModelImageNet (CNN or ViT).
...
class IMAGENET_VISION(BaseModelHarness):
    """
    Pattern:
      - One ImageNet train dataset and one ImageNet val dataset
    ...
    def __init__(self, cfg: Config, model: Optional[nn.Module] = None):
        super().__init__(cfg=cfg, model=VisionModelImageNet(cfg=cfg))

        # FULL datasets (no index split)
        self.ds_train = get_imagenet_train(cfg=cfg, normalize=True)
        self.ds_val = get_imagenet_val(cfg=cfg, normalize=True)
