...
class TestEnvelopeStructure:
    ...
    def test_every_registered_case_resolves_from_its_own_token(self):
        # Driven by the registry rather than a hard-coded token list, so a
        # device registered from the data root is covered too. The hard-coded
        # list is what let an unregistered device borrow the D3D envelope.
        ds = SOLPS2DwIONDataset.__new__(SOLPS2DwIONDataset)
        for case in _CASE_MINMAX:
            token = case.split("-", 1)[1]
            assert ds._infer_case(f"/some/path/{token}/run/b2time.nc") == case

    def test_unregistered_device_raises_instead_of_borrowing(self):
        ds = SOLPS2DwIONDataset.__new__(SOLPS2DwIONDataset)
        with pytest.raises(KeyError, match="No device token"):
            ds._infer_case("/some/path/no-such-machine/run/b2time.nc")
    ...
    def test_data_root_envelopes_are_registered(self):
        from examples.matey.solps.solps2dwion_dataset import register_envelopes

        register_envelopes({"testdev": {"ne": (1.0, 2.0)}})
        try:
            ds = SOLPS2DwIONDataset.__new__(SOLPS2DwIONDataset)
            assert ds._infer_case("/x/TESTDEV/b2time.nc") == "SOLPS-TESTDEV"
        finally:
            _CASE_MINMAX.pop("SOLPS-TESTDEV", None)
