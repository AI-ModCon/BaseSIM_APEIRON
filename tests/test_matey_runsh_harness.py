    ...
    *,
    ...
    class DummyYParams:
        def __init__(self, yaml_filename: str, config_name: str):
            ...
            if train_data_paths is not None and valid_data_paths is not None:
                ...
                self.train_data_paths = [["unused", "incompNS", "", "tk-2D"]]
                self.valid_data_paths = [["unused", "incompNS", "", "tk-2D"]]
            else:
    ...
    class DummyMateyCore(torch.nn.Module):
        ...
        def forward(self, inp, field_labels, bcs, opts):
            if isinstance(inp, torch.Tensor):
                return inp[-1].float() * self._weight
            if hasattr(inp, "y"):
                return inp.y.float() * self._weight
    ...
    def fake_get_data_loader(*args, **kwargs):
        ...
        if graph_batches:
            ...
            class DummyGraph:
                def __init__(self) -> None:
                    self.y = torch.randn(2, 4, 2, 2, 2)
                    self.leadtime = torch.ones(2, 1, dtype=torch.long)
    ...
    return {
        ...
        "determine_turt_levels": lambda tk_size, shape, imod: 0,
        "ForwardOptionsBase": DummyForwardOptionsBase,
        "autoregressive_rollout": (
            lambda model, inp, labels, bcs, opts, pushforward=True: (
...
@pytest.mark.parametrize(
    "update_mode", ["base", "none", "ewc_online", "kfac_online", "jvp_reg"]
)
def test_every_update_mode_is_accepted(update_mode: str) -> None:
    """The harness used to reject everything but base/none up front.

    Construction still fails here, on the missing data root -- that is the
    point: the update mode is no longer what stops it.
    """
    MATEYHarness, _, _ = _import_matey_symbols()
    cfg = _make_cfg(data_path="no_such_matey_root", update_mode=update_mode)
...
def test_data_root_requires_train_and_valid_when_one_exists(
    ...
    with pytest.raises(
        FileNotFoundError, match="must contain both 'train/' and 'valid/'"
    ):
        MATEYHarness(cfg)
...
def test_harness_builds_stream_and_loss_with_mocked_matey(
    ...
    train_loader, val_loader = harness.get_train_dataloaders()
